  ...
  <section id="faq" className="bg-[#0a0a0b] py-20 md:py-28 border-t border-[#1e1d1b]">
    <div className="max-w-5xl mx-auto px-5 md:px-8 grid lg:grid-cols-12 gap-10">
      <div className="lg:col-span-5">
        <span className="inline-block text-[11px] uppercase tracking-[0.32em] text-[#C5C8CC] mb-5">FAQ</span>
        ...
        <Accordion type="single" collapsible className="w-full">
          {FAQS.map((f, i) => (
            <AccordionItem key={i} value={`item-${i}`} className="border-b border-[#1e1d1b]">
              <AccordionTrigger className="text-left text-[17px] md:text-[19px] font-semibold text-[#F5F3EE] hover:text-[#C5C8CC] py-5 hover:no-underline tracking-tight">
                {f.q}
              </AccordionTrigger>
