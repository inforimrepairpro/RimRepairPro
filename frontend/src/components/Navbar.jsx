...
const Navbar = () => {
  ...
  return <header className={`fixed inset-x-0 top-0 z-50 border-b transition-all duration-300 ${scrolled ? 'border-[#d8aa42]/20 bg-[#090b0d]/95 shadow-2xl backdrop-blur-xl' : 'border-white/10 bg-[#090b0d]/88 backdrop-blur-md'}`}>
    <div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between gap-4 px-4 sm:px-5 md:h-[82px] md:px-8">
      ...
      <nav className="hidden items-center gap-6 xl:flex">{NAV_LINKS.map(l => <a key={l.href} href={l.href} className="nav-link text-[11px] font-semibold text-zinc-300 transition hover:text-[#f0c75e]">{l.label}</a>)}</nav>
