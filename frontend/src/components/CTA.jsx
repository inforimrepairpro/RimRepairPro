...
import { Camera, CalendarDays, MessageSquareText, Phone, MapPin, ShieldCheck, BadgeDollarSign } from 'lucide-react';
    ...
    <section id="contact" className="bg-[#081526] py-20 md:py-28 relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-5 md:px-8 grid lg:grid-cols-12 gap-10 lg:gap-16">
        <div className="lg:col-span-5">
          ...
          <h2 className="mt-5 font-display text-[52px] md:text-[76px] leading-[.94] font-bold text-white">Get Quote<br/><span className="text-[#64d8ff]">or Book Now.</span></h2>
          <p className="mt-6 text-[17px] leading-relaxed text-slate-300 max-w-lg">Send photos of your rim plus your service address. We review the damage and text you back with your quote and scheduling details.</p>

          <div className="mt-8 grid sm:grid-cols-3 lg:grid-cols-1 gap-3">
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.05] p-4"><BadgeDollarSign className="w-5 h-5 shrink-0 text-[#64d8ff]"/><div><div className="font-bold text-white">No Deposit</div><div className="text-xs text-slate-400">Book without paying a deposit</div></div></div>
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.05] p-4"><MapPin className="w-5 h-5 shrink-0 text-[#64d8ff]"/><div><div className="font-bold text-white">No Distance Fee</div><div className="text-xs text-slate-400">No separate travel charge for distance</div></div></div>
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.05] p-4"><ShieldCheck className="w-5 h-5 shrink-0 text-[#64d8ff]"/><div><div className="font-bold text-white">Work Warranty</div><div className="text-xs text-slate-400">Our completed work is backed by our warranty</div></div></div>
          </div>
          ...
          <div className="mt-8 space-y-4 text-sm text-slate-300">
            ...
            <div className="flex items-center gap-3"><MessageSquareText className="w-5 h-5 text-[#64d8ff]"/> Receive your quote and scheduling details by text</div>
          </div>
        ...
        <div className="lg:col-span-7 rounded-[30px] bg-[#f7f3eb] p-5 md:p-8 shadow-2xl">
          <div className="grid grid-cols-2 gap-2 rounded-2xl bg-[#e8e4dc] p-1.5">
            ...
            <button onClick={() => setTab('book')} className={`rounded-xl px-3 py-3.5 text-[10px] md:text-[11px] font-black uppercase tracking-[.12em] transition ${tab === 'book' ? 'bg-[#2f7cff] text-white shadow' : 'text-slate-600'}`}><CalendarDays className="inline w-4 h-4 mr-2"/>Book Your Service</button>
          </div>

          {tab === 'book' && <div className="mt-5 rounded-xl border border-[#2f7cff]/15 bg-[#2f7cff]/5 px-4 py-3 text-xs font-semibold text-[#081526]">No deposit is required to request your appointment.</div>}
          ...
          <a href={`sms:${BRAND.phoneTel}`} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-[#2f7cff] px-5 py-4 text-sm font-black text-white hover:bg-[#438aff] transition"><MessageSquareText className="w-5 h-5"/>{tab === 'quote' ? 'Send & Get Quote via Text' : 'Request Appointment via Text'}</a>
          <p className="mt-3 text-center text-[11px] text-slate-500">Your phone can open a text to {BRAND.phone}. No deposit required.</p>
        </div>
      </div>
    </section>
  );
